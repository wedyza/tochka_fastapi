...
def delete_user(
    ...
    admin_id: str = Depends(oauth2.require_admin),
...
@router.post(
    "/instrument",
    response_model=schemas.InstrumentResponse,
    status_code=status.HTTP_201_CREATED,
)
def add_instrument(
    payload: schemas.InstrumentCreateSchema,
    db: Session = Depends(get_db),
    admin_id: str = Depends(oauth2.require_admin),
) -> schemas.InstrumentCreateSchema:
    instrument = (
        db.query(models.Instrument)
        .filter(
            and_(
                or_(
                    models.Instrument.name == payload.name,
...
def deposit(
    ...
    instrument = (
        db.query(models.Instrument)
        .filter(models.Instrument.ticker == payload.ticker)
